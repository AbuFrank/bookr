...
App state is split across two contexts, nested `SessionProvider` outside `DataProvider` in `App.tsx`:

- **`context/sessionContext.tsx`** (`SessionProvider`, read via `hooks/useAuth.ts`) — Firebase-only: `user`, `isAuthenticated`, `loading` (true until Firebase resolves whether a session exists), `loginWithGoogle`, `logout`. Nothing here touches Firestore.
- **`context/dataContext.tsx`** (`DataProvider`, read via `hooks/useData.ts`) — the four `useReducer` stores (`reducer/{transaction,account,ledger,folder}Reducer.tsx`), each with its own Action-type enum in the matching `types/*.ts` file, plus `hasUnsavedReportChanges`. `DataProvider` reads `user` from `SessionContext` internally (it must be mounted inside `SessionProvider`) and runs an effect keyed on it: when `user` becomes non-null, transactions/accounts/ledgers/folders are loaded in parallel and dispatched into their reducers (`dataLoading` is true until this resolves); when `user` goes back to null (sign-out, however it happens — not just the explicit logout button), all four reducers are reset. There is no other global state.

Most components only need one of the two hooks; a few (`Header.tsx`, `ReportTrigger.tsx`, `PageBooks.tsx`, `PageTransactions.tsx`) call both, e.g. `user`/`logout` from `useAuth()` alongside `hasUnsavedReportChanges`/`accounts`/etc. from `useData()`. Watch for the `loading` naming split: `SessionContext.loading` (auth resolving, used by `ProtectedRoute`) and `DataContext.dataLoading` (initial Firestore fetch, used by `PageTransactions`) mean different things — don't conflate them.

`updateBooks(groupFolder, yearFolder)` (in `DataContext`) is the pivot when switching book/fiscal year: it filters accounts by `bookId` and ledgers by `parentFolderId`, then sets the most-recently-created ledger in that year as `currentLedger`.

The account and ledger reducers each keep a full list (`accounts`/`ledgers`) plus a book/year-scoped "current" list (`currentAccounts`/`currentLedgers`) that `updateBooks` derives from the full list — but the reducers themselves only ever touch the full list on add/update/delete, so every mutating action in `dataContext.tsx` (`addAccount`/`updateAccount`/`deleteAccount`, `addLedger`/`updateLedger`) has to separately dispatch a `SET_CURRENT_*` update, or the change won't show up anywhere in the UI until the next book/year switch. Folders are the exception — `folderReducer.tsx` updates `currentChildren` inside the reducer itself. Transactions don't need this at all: `currentTransactions` is derived by a `useEffect` keyed off `ledgerState.currentLedger`, so it re-syncs automatically.
...
`hasUnsavedReportChanges` (in `dataContext.tsx`) tracks whether the Google Sheet report is stale relative to Firestore: `addTransaction`/`updateTransaction`/`deleteTransaction` set it `true`, and so does `updateFolder` (a fiscal year's `startingBalance` seeds its chronologically-first ledger's running total, same as a transaction edit — see `calculateAccountTotals`). It's only cleared by `markReportSaved()`, which `ReportTrigger` calls after `/update-sheets` reports every ledger write succeeded (a partial failure leaves it `true`). It gates the "Update Report" button's disabled state, drives a `beforeunload` browser warning, and guards the Header's sign-out action with a confirm — so it has to actually reach `false` on a real successful sync, not just after the request completes.
...
- `getAuthenticatedEmail()` (`server/firebaseAuth.js`) wraps `verifyIdToken()` in a blanket `try/catch` that flattens *any* thrown error — a genuinely bad/expired token, a missing `Authorization` header reaching the wrong branch, or `getAdminApp()`'s own `"FIREBASE_PROJECT_ID environment variable is required"` error if that env var is unset — into the same generic 401 `Invalid or expired token`. A `FIREBASE_PROJECT_ID` that's missing, or set to the wrong project (e.g. the GCP service-account project instead of the Firebase project — see Local server setup above), looks identical client-side to an expired token, including right after a fresh sign-in; the real cause only shows up server-side via the `console.error('verifyIdToken failed:', err)` line right before the throw.
- `firebase-admin` pulls in `jwks-rsa@4.1.0`, which depends on `jose@^6` — an ESM-only release with no CommonJS build. `jwks-rsa` still does a plain `require('jose')`, which crashes with `ERR_REQUIRE_ESM` under Vercel's Node runtime the moment anything imports `firebase-admin/auth` (i.e. any route through `server/firebaseAuth.js`), even though it can still work locally depending on the environment. Pinned via an npm `overrides` entry in `package.json` (`jwks-rsa` → `jose@^5.10.0`, the last version with a CJS build); `jwks-rsa` only calls `jose.importJWK`/`jose.exportSPKI`, unchanged between v5 and v6. Re-check this override on any `firebase-admin`/`jwks-rsa` upgrade.
