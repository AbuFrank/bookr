...
- Close the remaining gap from the above: being *some* authenticated Bookr user wasn't enough to stop one user's login from referencing another user's Drive folder/ledger by id. New `assertAuthorizedForFileIds()`/`isEmailAuthorizedForFile()` (`server/google-auth.js`) check, via `drive.permissions.list`, that the authenticated email already has Drive access to the id(s) a request references — reusing the reader-access-on-create model `createFolder`/`copyTemplateFile` already set up, so no new Firestore Admin credential was needed. `/folder` checks `parentId` (when given), `/copy-template` checks `parentFolderId`, and `/update-sheets` checks every `fileId` in the batch before writing any of them, rejecting the whole batch on any 403 rather than partially applying it.
